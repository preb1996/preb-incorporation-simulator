...
import { CalculationError, integer, months, yen, rate, scale, add, subtract, min, max, sum, compare, type Fraction } from './core.ts';
import {
 truncateIndividualBusinessTaxableBase,truncateIndividualBusinessTaxAmount,
 truncateCorporationTaxableIncome,truncateCorporationTaxCalculatedAmount,truncateCorporationTaxAmount,
 truncatePrefecturalCorporateTaxBase,truncatePrefecturalCorporateIncomeTax,truncatePrefecturalCorporatePerCapitaTax,
 truncateMunicipalCorporateTaxBase,truncateMunicipalCorporateIncomeTax,truncateMunicipalCorporatePerCapitaTax,
 truncateCorporateEnterpriseTaxableIncome,truncateCorporateEnterpriseTaxAmount,truncateSpecialCorporateEnterpriseTaxAmount,
 truncateConsumptionTaxIntermediate,truncateConsumptionTaxPayable
} from './rounding.ts';
export function individualBusinessTaxRaw(beforeBlue:number,bps:number,businessMonths:number):Fraction {
 integer(beforeBlue,'beforeBlue',true);months(businessMonths);
...
export function individualBusinessTax2026(beforeBlue:number,bps:number,businessMonths:number) {
 integer(beforeBlue,'beforeBlue',true);months(businessMonths);
 if(!master.individualBusinessTax.values.allowedBps.includes(bps))throw new CalculationError('INVALID_INPUT','businessTaxRate');
...
export function corporationTaxThreshold(businessMonths=12):Fraction {
 months(businessMonths);return scale(yen(master.corporationTax.values.threshold),businessMonths,12);
}
export function corporationTaxRaw(income:number,businessMonths=12):Fraction {
 integer(income,'income',true);const m=master.corporationTax.values;const base=yen(Math.max(0,income)),threshold=corporationTaxThreshold(businessMonths);
 return add(rate(min(base,threshold),m.lowerBps),rate(max(yen(0),subtract(base,threshold)),m.upperBps));
}
export function corporationTax2026(income:number,businessMonths=12) {
 integer(income,'income',true);months(businessMonths);
...
export function corporateEnterpriseTaxRaw(income:number,businessMonths:number) {
 integer(income,'income',true);months(businessMonths);const m=master.enterprise.values;
...
export function corporateEnterpriseTax2026(income:number,businessMonths:number) {
 integer(income,'income',true);months(businessMonths);
...
/** Takes an already decomposed legal period; does not silently choose date/calendar conventions. */
export interface MonthParts {wholeMonths:number;remainingDays:number;}
function validatePeriod(p:MonthParts) {
 integer(p.wholeMonths,'wholeMonths');integer(p.remainingDays,'remainingDays');
 if(p.wholeMonths>12||p.remainingDays>30||(p.wholeMonths===0&&p.remainingDays===0)||(p.wholeMonths===12&&p.remainingDays>0))throw new CalculationError('INVALID_INPUT','period');
}
export function enterpriseTaxBusinessMonths(period:MonthParts):number {validatePeriod(period);return period.wholeMonths+(period.remainingDays>0?1:0);}
export function perCapitaTaxPresenceMonths(period:MonthParts):number {validatePeriod(period);return period.wholeMonths===0?1:period.wholeMonths;}
export function corporateLocalTaxRaw(capital:number,corporationTaxBase:Fraction,presenceMonths:number) {
 integer(capital,'capital');months(presenceMonths);const m=master.corporateLocal.values;
...
export function corporateLocalTax2026(capital:number,corporationTaxAmount:number,presenceMonths:number) {
...
export type ConsumptionInput = {status:'EXEMPT'} | {status:'GENERAL';output:number;deductibleInput:number} | {status:'SIMPLIFIED';output:number;deemedPurchaseBps:number} | {status:'SPECIAL_20_PERCENT';output:number;eligible:boolean} | {status:'MANUAL';amount:number};
export function consumptionTaxRaw(input:ConsumptionInput):Fraction {
 switch(input.status){
 case 'EXEMPT':return yen(0);
 case 'GENERAL':integer(input.output,'output');integer(input.deductibleInput,'deductibleInput');return subtract(yen(input.output),yen(input.deductibleInput));
 case 'SIMPLIFIED':integer(input.output,'output');integer(input.deemedPurchaseBps,'deemedPurchaseBps');if(input.deemedPurchaseBps>10000)throw new CalculationError('INVALID_INPUT','deemedPurchaseBps');return rate(yen(input.output),10000-input.deemedPurchaseBps);
 case 'SPECIAL_20_PERCENT':if(input.eligible!==true)throw new CalculationError('INVALID_INPUT','2割特例の適用要件未充足');integer(input.output,'output');return rate(yen(input.output),master.consumption.values.specialBps);
 case 'MANUAL':integer(input.amount,'amount',true);return yen(input.amount);
 default:throw new CalculationError('INVALID_INPUT','consumptionTaxStatus');
 }
}
export function consumptionTax2026(input:ConsumptionInput) {
...
export function firstPeriodExemptionCandidate(capital:number,invoiceRegistered:boolean,taxableBusinessElection:boolean,specificNewCorporationFlag:boolean):boolean {
 integer(capital,'capital');
 for(const value of [invoiceRegistered,taxableBusinessElection,specificNewCorporationFlag])if(typeof value!=='boolean')throw new CalculationError('INVALID_INPUT','免税候補判定には全フラグ必須');
