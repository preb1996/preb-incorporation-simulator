...
import { CalculationError, integer, yen, rate, add, subtract, min, max, exactInteger, type Fraction } from './core.ts';
export function businessIncome(sales:number, expenses:number, blue:number) {
  integer(sales,'sales'); integer(expenses,'expenses');
  if (![0,550000,650000].includes(blue)) throw new CalculationError('INVALID_INPUT','blueReturnDeduction');
...
function deductionFromTable(income:number, rows:number[][]):number {
  integer(income,'income',true);
  for (const row of rows) { const [ceiling,deduction]=row; if (ceiling===undefined||deduction===undefined) throw new Error('Invalid master'); if(income<=ceiling)return deduction; }
  return 0;
}
export function incomeTaxBasicDeduction(income:number):number { return deductionFromTable(income,master.incomeTax.values.basic); }
export function nhiBasicDeduction(previousTotalIncome:number):number { return deductionFromTable(previousTotalIncome,master.nhi.values.basic); }
/** Caller supplies the already legally rounded taxable income; no unapproved deduction or rounding is inferred. */
export function baseIncomeTax(taxableIncome:number):number {
  ...
  for(const row of master.incomeTax.values.brackets) { const [exclusive,r,d]=row; if(exclusive!==undefined&&taxableIncome<exclusive){bps=r;deduction=d;break;} }
  if(bps===undefined||deduction===undefined)throw new Error('Invalid master');
  return exactInteger(max(yen(0),subtract(rate(yen(taxableIncome),bps),yen(deduction))));
}
export function reconstructionTax(base:number) {
...
export function salaryIncomeRaw(revenue:number) {
  ...
  for(const row of master.salary.values.brackets) {
    ...
    if(ceiling===undefined||bps===undefined||fixed===undefined)throw new Error('Invalid master');
    if(revenue<=ceiling){deduction=add(rate(yen(revenue),bps),yen(fixed));break;}
  }
...
export function adjustmentDeduction(totalTaxableIncome:number,personalDifference:number,totalIncome:number) {
  integer(totalTaxableIncome,'totalTaxableIncome');integer(personalDifference,'personalDifference');integer(totalIncome,'totalIncome',true);
...
export function residentIncomeTaxRaw(taxableIncome:number, municipalAdjustment:Fraction,prefecturalAdjustment:Fraction,otherMunicipalCredits:number,otherPrefecturalCredits:number) {
 integer(taxableIncome,'taxableIncome');integer(otherMunicipalCredits,'credits');integer(otherPrefecturalCredits,'credits');
...
export function residentBasicDeduction2026(totalIncome:number):number {
...
/**
 * Finalizes the Nagoya/Aichi annual levy after exemption has been determined.
 * The caller supplies the exemption result because family/dependent inputs are
 * outside the v1.0 minimum input set.
 */
export type ResidentTaxExemptionStatus='FULL'|'INCOME_ONLY'|'NONE';
export function residentTaxExemption2026(input:{totalIncome:number;grossTotalIncome:number;dependentCount:number;receivesWelfare?:boolean;isDisabledMinorWidowOrSingleParent?:boolean}):ResidentTaxExemptionStatus {
 const {totalIncome,grossTotalIncome,dependentCount}=input;
 integer(totalIncome,'totalIncome',true);integer(grossTotalIncome,'grossTotalIncome',true);integer(dependentCount,'dependentCount');
 ...
 if(input.receivesWelfare||(input.isDisabledMinorWidowOrSingleParent&&totalIncome<=1350000)||totalIncome<=fullLimit)return 'FULL';
 if(grossTotalIncome<=incomeLimit)return 'INCOME_ONLY';
 return 'NONE';
}

export function residentTax2026(input:{taxableIncome:number;totalIncome:number;personalDeductionDifferenceTotal:number;otherMunicipalCredits?:number;otherPrefecturalCredits?:number;exemptionStatus:ResidentTaxExemptionStatus}) {
 const {taxableIncome,totalIncome,personalDeductionDifferenceTotal,exemptionStatus}=input;
 integer(taxableIncome,'taxableIncome');integer(totalIncome,'totalIncome',true);integer(personalDeductionDifferenceTotal,'personalDeductionDifferenceTotal');
 if(taxableIncome%1000!==0)throw new CalculationError('INVALID_INPUT','住民税課税所得は1,000円未満切捨て後を指定');
 const otherMunicipalCredits=input.otherMunicipalCredits??0,otherPrefecturalCredits=input.otherPrefecturalCredits??0;
 integer(otherMunicipalCredits,'otherMunicipalCredits');integer(otherPrefecturalCredits,'otherPrefecturalCredits');
