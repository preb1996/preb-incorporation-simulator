...
import { block, CalculationError, integer, months, yen, rate, add, min, sum, exactInteger } from './core.ts';
import { nhiBasicDeduction } from './personal.ts';
export interface NhiPerson { businessIncomeAfterBlueDeduction:number; previousTotalIncome:number; age:number; }
export type HouseholdNhiPayer='HUSBAND'|'WIFE';
/** 国税庁No.1130: 社会保険料控除は実際に支払った者へ全額帰属。支払者を推測しない。 */
export function householdNhiDeductionAllocation(totalPremium:number,payer?:HouseholdNhiPayer) {
 integer(totalPremium,'householdNationalHealthInsurance');
 if(payer===undefined)throw new CalculationError('INVALID_INPUT','householdNhiPayer is required');
...
export function householdNhiRaw(people:readonly [NhiPerson,NhiPerson]) {
 ...
 const part=(base:number,count:number,terms:{fixed:number;bps:number;cap:number})=>min(yen(terms.cap),add(yen(terms.fixed*count),rate(yen(base),terms.bps)));
...
function truncatePositiveFractionToTen(value:{numerator:bigint;denominator:bigint}):number {
...
export function householdNhi2026(people:readonly [NhiPerson,NhiPerson]) {
...
export function nationalPension(eligibleMonths:number):number {integer(eligibleMonths,'eligibleMonths');if(eligibleMonths>12)throw new CalculationError('INVALID_INPUT','eligibleMonths');return master.nationalPension.values.monthly*eligibleMonths;}
export function idecoLimit(month:number,category:'FIRST'|'SECOND',corporatePensionContribution=0):number {
 months(month);integer(corporatePensionContribution,'corporatePensionContribution');
 if(category!=='FIRST'&&category!=='SECOND')throw new CalculationError('INVALID_INPUT','category');
 const m=month<12?master.idecoBefore.values:master.idecoAfter.values;
 if(category==='FIRST')return m.first;
 if(month<12&&corporatePensionContribution!==0)return block('11月以前の企業年金ありは未定義');
 if(corporatePensionContribution>m.second)throw new CalculationError('INVALID_INPUT','企業年金が合算上限を超過');
 return m.second-corporatePensionContribution;
}
export function annualIdeco(requested:readonly number[],incorporationMonth:number|null) {
 if(requested.length!==12)throw new CalculationError('INVALID_INPUT','12か月分必須');
 if(incorporationMonth!==null)months(incorporationMonth);
...
export type SocialInsuranceSystem = 'HEALTH' | 'PENSION';
interface Grade {grade:number;remunerationLowerInclusive:number|null;remunerationUpperExclusive:number|null;standardMonthlyRemuneration:number;}
function findGrade<T extends Grade>(salary:number, grades:readonly T[]):T {
...
export function remunerationGrade(salary:number,system:SocialInsuranceSystem) {
  if(system==='HEALTH')return findGrade(salary,socialMaster.healthInsuranceGrades);
  if(system==='PENSION')return findGrade(salary,socialMaster.employeesPensionGrades);
...
export function officeTotalPremiumHundredths(amounts:readonly number[]):number {
...
export interface SocialInsurancePerson {monthlyExecutiveSalary:number;age:number;}
type ContributionKey='healthInsurance'|'nursingCare'|'childrenSupport'|'employeesPension';
interface ExactContribution {fullHundredthsYen:number;employeeYen:number;}
interface MonthlyPersonContribution {healthStandardMonthlyRemuneration:number;pensionStandardMonthlyRemuneration:number;healthInsurance:ExactContribution;nursingCare:ExactContribution;childrenSupport:ExactContribution;employeesPension:ExactContribution;}
function exactContribution(standard:number,bps:number,applies:boolean):ExactContribution {
...
function closeSystem(people:readonly ExactContribution[]) {
...
function socialInsuranceRates(year:number,month:number) {
...
function calculateMonthlySocialInsuranceForActivePeople(people:readonly SocialInsurancePerson[],year:number,month:number) {
  integer(year,'year');months(month);
  ...
  const calculatePerson=(person:SocialInsurancePerson):MonthlyPersonContribution=>{
  ...
  function aggregate(key:ContributionKey){return closeSystem(perPerson.map(person=>person[key]));}
...
export function calculateMonthlySocialInsurance(people:readonly [SocialInsurancePerson,SocialInsurancePerson],year:number,month:number) {
 const result=calculateMonthlySocialInsuranceForActivePeople(people,year,month);
 return {...result,perPerson:result.perPerson as [MonthlyPersonContribution,MonthlyPersonContribution]};
}

export interface InitialYearSocialInsurancePerson {age:number;qualificationDate:string;monthlyExecutiveSalaryActuals:readonly number[];}
function qualificationMonth2026(value:string):number {
 const match=/^(2026)-(\d{2})-(\d{2})$/.exec(value);
 if(!match)throw new CalculationError('INVALID_INPUT','socialInsuranceQualificationDate');
 const month=Number(match[2]),day=Number(match[3]),date=new Date(Date.UTC(2026,month-1,day));
 if(date.toISOString().slice(0,10)!==value)throw new CalculationError('INVALID_INPUT','socialInsuranceQualificationDate');
 return month;
}
/** 資格取得月から、実際の月額報酬と該当月の公式率で計算し、年額へ合算する。 */
export function calculateInitialYearSocialInsurance(people:readonly [InitialYearSocialInsurancePerson,InitialYearSocialInsurancePerson]) {
 const qualificationMonths=people.map(person=>qualificationMonth2026(person.qualificationDate));
 for(const person of people){integer(person.age,'age');if(person.monthlyExecutiveSalaryActuals.length!==12)throw new CalculationError('INVALID_INPUT','12か月分の役員報酬実額が必要');person.monthlyExecutiveSalaryActuals.forEach(value=>integer(value,'monthlyExecutiveSalaryActual'));}
 ...
 for(let month=1;month<=12;month++){
  ...
  activeIndexes.forEach((originalIndex,activeIndex)=>{employeeByPerson[originalIndex]!+=Object.values(result.perPerson[activeIndex]!).filter(value=>typeof value==='object').reduce((total,item)=>total+(item as ExactContribution).employeeYen,0);});
  for(const key of Object.keys(employerBySystem) as ContributionKey[])employerBySystem[key]+=result.systems[key].employerYen;
