import { CalculationError, integer } from './core.ts';

export type AnnualSystem='INCOME_TAX'|'RESIDENT_TAX'|'NATIONAL_HEALTH_INSURANCE'|'INDIVIDUAL_BUSINESS_TAX';

/** Returns the calendar income year referenced by an assessment year. */
export function referencedIncomeYear(system:AnnualSystem,assessmentYear:number):number {
 integer(assessmentYear,'assessmentYear');
 switch(system){
  case 'INCOME_TAX':case 'INDIVIDUAL_BUSINESS_TAX':return assessmentYear;
  case 'RESIDENT_TAX':case 'NATIONAL_HEALTH_INSURANCE':return assessmentYear-1;
 }
...
export function splitMonthlyAmountsAtIncorporation(monthly:readonly number[],incorporationMonth:number) {
 if(monthly.length!==12)throw new CalculationError('INVALID_INPUT','12か月分の実額が必要');
 integer(incorporationMonth,'incorporationMonth');
 if(incorporationMonth<1||incorporationMonth>12)throw new CalculationError('INVALID_INPUT','incorporationMonth');
...
export interface InitialYearFacts {
 corporationEstablishmentDate:string;
 fiscalYearStart:string;
 fiscalYearEnd:string;
 socialInsuranceQualificationDate:{husband:string;wife:string};
 executiveSalaryMonthlyActuals:{husband:readonly number[];wife:readonly number[]};
 corporateSalesMonthlyActuals:readonly number[];
 corporateOperatingExpensesMonthlyActuals:readonly number[];
 employerSocialInsuranceMonthlyActuals:readonly number[];
 householdNationalHealthInsuranceMonthlyActuals:readonly number[];
 nationalPensionMonthlyActuals:{husband:readonly number[];wife:readonly number[]};
 nationalHealthInsuranceLossMonth:{husband:string;wife:string};
 nationalPensionCategory2SwitchMonth:{husband:string;wife:string};
 corporatePerCapitaPresenceMonths:number;
 enterpriseTaxBusinessMonths:number;
}
function isoDate(value:string,label:string):string {
 const match=/^(\d{4})-(\d{2})-(\d{2})$/.exec(value);
 if(!match)throw new CalculationError('INVALID_INPUT',label);
 const year=Number(match[1]),month=Number(match[2]),day=Number(match[3]),date=new Date(Date.UTC(year,month-1,day));
 if(date.getUTCFullYear()!==year||date.getUTCMonth()!==month-1||date.getUTCDate()!==day)throw new CalculationError('INVALID_INPUT',label);
 return value;
}
function yearMonth(value:string,label:string):string {
 if(!/^\d{4}-(0[1-9]|1[0-2])$/.test(value))throw new CalculationError('INVALID_INPUT',label);
 return value;
}
function annualActuals(values:readonly number[],label:string):number {
 if(values.length!==12)throw new CalculationError('INVALID_INPUT',`${label}: 12か月分の実額が必要`);
 return values.reduce((total,value)=>total+integer(value,label),0);
}
/** 日割り・年額/12を一切行わず、別々に入力された制度日付と月別実額を保持・合算する。 */
export function initialYearActuals(input:InitialYearFacts) {
 const timeline={
 ...
 if(timeline.fiscalYearStart>timeline.fiscalYearEnd||timeline.corporatePerCapitaPresenceMonths<1||timeline.corporatePerCapitaPresenceMonths>12||timeline.enterpriseTaxBusinessMonths<1||timeline.enterpriseTaxBusinessMonths>12)throw new CalculationError('INVALID_INPUT','initialYearTimeline');
 return {timeline,
